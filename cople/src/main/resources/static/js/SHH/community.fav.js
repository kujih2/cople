$(function(){
	...
	$('#output_fav').click(function(){
		...
		console.log('like_status:', $(this).attr('data-favStatus'));
		
		$.ajax({
			url:'writeFav',
			type:'post',
			data:{board_num:$('#output_fav').attr('data-num'),
				  fav_status:$('#output_fav').attr('data-favStatus'),
				  },
			dataType:'json',
			success:function(param){
				if(param.result == 'logout'){
	...
	function displayFav(param){
		let output,output2;
		if(param.status=='fav'){//좋아요 선택
			output = '../images/fav_alt.png';
			output2 = '../images/hate_alt.png';
		}else if(param.status == 'hate'){//싫어요 선택
			output = '../images/fav_alt.png';
			output2 = '../images/hate_alt.png';
